...
void startCameraServer() {
  ...
  sensor_t *s = esp_camera_sensor_get();
  if (s != NULL) {
    s->set_framesize(s, FRAMESIZE_VGA);  // Set resolution to VGA (640x480)
    s->set_quality(s, 4);                // Set quality to 4
    s->set_brightness(s, -2);            // Set brightness to -2
    s->set_hmirror(s, 1);                // Enable H-mirror
  }
